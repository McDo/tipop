...
CGRect POPLayerGetRotation3D(CALayer *l)
{
    DECOMPOSE_TRANSFORM(l);
    return CGRectMake(_d.rotateX, _d.rotateY, _d.rotateZ, -1);
}

void POPLayerSetRotation3D(CALayer *l, CGRect p)
{
    DECOMPOSE_TRANSFORM(l);
    _d.rotateX = p.origin.x;
    _d.rotateY = p.origin.y;
    _d.rotateZ = p.size.width;
    RECOMPOSE_ROT_TRANSFORM(l);
}
